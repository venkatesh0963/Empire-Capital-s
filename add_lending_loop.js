...
text = text.replace('let updatedLoans = [...state.banking.loans];', logic + '\n        let updatedLoans = [...state.banking.loans];');

// Then update state.banking in advanceDay return
text = text.replace('banking: { ...state.banking, loans: updatedLoans },', 'banking: { ...state.banking, loans: updatedLoans, lendedLoans: updatedLendedLoans, loanApplications: updatedLoanApps },');

fs.writeFileSync('src/store/gameStore.ts', text);
console.log('Fixed advanceDay logic!');
