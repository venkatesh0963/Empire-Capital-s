...
with open('src/components/Business.tsx', 'r', encoding='utf-8') as f:
    text = f.read()

# Make sure we add expandBusiness to the destructuring assignment
if "expandBusiness" not in text:
    text = text.replace(
        "const { player, business, createBusiness, updateBusinessSettings, hireEmployee, fireEmployee } = useGameStore();",
        "const { player, business, createBusiness, updateBusinessSettings, hireEmployee, fireEmployee, expandBusiness } = useGameStore();"
    )

# Replace the business card rendering block inside the 'manage' tab
marker_start = "                  return (\n                    <div key={biz.id} className=\"bg-brand-card rounded-xl border border-black/10 overflow-hidden shadow-sm hover:shadow-md transition-shadow\">"
marker_end = "                      </div>\n                    </div>\n                  );\n                })}\n              </div>"
...
replacement = """                  const expansionCost = (biz.level || 1) * 50000;
                  return (
                               ...
                               if (exec) {
                                  return (
                                     <div key={exec.id} className="bg-brand-surface border border-brand-gold/20 rounded p-2 text-center relative group">
                                        ...
                                        <button onClick={() => useGameStore.getState().fireExecutive(biz.id, exec.id)} className="absolute inset-0 bg-brand-loss/90 text-white text-xs font-bold rounded opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">FIRE</button>
                                     </div>
                                  );
                               }
                               return (
                                  <button key={role} onClick={() => useGameStore.getState().hireExecutive(biz.id, role)} className="bg-brand-surface/50 hover:bg-brand-surface border border-black/5 border-dashed rounded p-2 text-center text-[10px] font-bold text-brand-muted transition">
                                     + HIRE {role}
                                  </button>
                               );
                            })}
...
if pattern.search(text):
    ...
    with open('src/components/Business.tsx', 'w', encoding='utf-8') as f:
        f.write(text)
    print("Re-styled operational card successfully!")
else:
    print("Could not find the marker pattern!")
